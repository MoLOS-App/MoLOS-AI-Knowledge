...
const LEVEL_SETTINGS: Record<HumanizationLevelType, { temperature: number; frequencyPenalty: number }> = {
...
const injectContractions = (text: string) =>
  CONTRACTION_SWAPS.reduce((acc, [pattern, replacement]) => acc.replace(pattern, replacement), text);
...
const injectBurstiness = (text: string, level: HumanizationLevelType) => {
  ...
  while (i < sentences.length) {
    ...
    if (words.length > targetLong) {
      ...
      if (splitIndex > 0) {
        output.push(words.slice(0, splitIndex + 1).join(" ").replace(/[;,]$/, "."));
...
const callProvider = async (
  ...
  return provider === AiProvider.ANTHROPIC ? parseAnthropicResponse(data) : parseOpenAiResponse(data);
};
...
export const runHumanizerPipeline = async ({
  ...
  if (canCallLlm) {
    ...
    stage1 = (await callProvider(provider, apiToken, primaryModel, [{ role: "user", content: deoptPrompt }], baseSettings)).message.trim();
    ...
    stage2 = (await callProvider(provider, apiToken, secondaryModel, [{ role: "user", content: burstPrompt }], burstSettings)).message.trim();

    const checkerModel = resolveModel(provider, providerSettings, "checker");
    const checkPrompt = buildPreservationPrompt(base, stage2);
    const checkResponse = await callProvider(provider, apiToken, checkerModel, [{ role: "user", content: checkPrompt }], {
      temperature: 0.2,
      frequencyPenalty: 0,
      topP: overrideSettings?.topP,
      maxTokens: overrideSettings?.maxTokens,

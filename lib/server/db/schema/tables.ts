...
export const abTests = sqliteTable('MoLOS-AI-Knowledge_ab_tests', {
	...
	status: textEnum('status', AbTestStatus).notNull().default(AbTestStatus.DRAFT),
	createdAt: integer('created_at').notNull().default(nowSeconds),
	updatedAt: integer('updated_at').notNull().default(nowSeconds)
});

export const usageAnalytics = sqliteTable('MoLOS-AI-Knowledge_usage_analytics', {
	id: text('id')
		.primaryKey()
		.$defaultFn(() => crypto.randomUUID()),
	userId: text('user_id').notNull(),
...
export const promptDeployments = sqliteTable('MoLOS-AI-Knowledge_prompt_deployments', {
	id: text('id')
		.primaryKey()
		.$defaultFn(() => crypto.randomUUID()),
	promptId: text('prompt_id')

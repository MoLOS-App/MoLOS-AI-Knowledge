...
import type { PlaygroundSession } from "../../../models";
	...
	async update(
		...
		const [result] = await this.db
			...
			.set({
				promptId: data.promptId,
				model: data.model,
				settingsJson: data.settings !== undefined ? toJsonString(data.settings, '{}') : undefined,
				messagesJson: data.messages !== undefined ? toJsonString(data.messages, '[]') : undefined,
				totalTokens: data.totalTokens,
				totalCost: data.totalCost,
				latencyMs: data.latencyMs,
				updatedAt: Math.floor(Date.now() / 1000)
